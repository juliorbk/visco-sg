...
class StatsServiceTest {
    ...
    private ProductRepository.CriticalProductProjection buildProjection(Long id, String name, String sku,
                                                                        BigDecimal currentStock, BigDecimal reorderPoint) {
        return new ProductRepository.CriticalProductProjection() {
            @Override public Long getProductId() { return id; }
            @Override public String getProductName() { return name; }
            @Override public String getSku() { return sku; }
            @Override public BigDecimal getCurrentStock() { return currentStock; }
            @Override public BigDecimal getReorderPoint() { return reorderPoint; }
        };
    }
    ...
    void shouldReturnKpis_whenDataExists() {
        ...
        when(productRepository.getTotalInventoryUnits()).thenReturn(new BigDecimal("5000"));
        when(orderRepository.getMonthlySpending(org.mockito.ArgumentMatchers.any()))
                .thenReturn(List.of(
                        new PurchaseOrderRepository.MonthlySpendingProjection() {
                            @Override public java.time.LocalDate getMonth() { return java.time.LocalDate.now(); }
                            @Override public BigDecimal getTotal() { return new BigDecimal("15000.00"); }
                        }
                ));
    ...
    void shouldReturnKpisWithNullMonthlySpend_whenSpendingIsNull() {
        ...
        when(productRepository.getTotalInventoryUnits()).thenReturn(new BigDecimal("100"));
        when(orderRepository.getMonthlySpending(org.mockito.ArgumentMatchers.any()))
                .thenReturn(List.of(
                        new PurchaseOrderRepository.MonthlySpendingProjection() {
                            @Override public java.time.LocalDate getMonth() { return java.time.LocalDate.now(); }
                            @Override public BigDecimal getTotal() { return null; }
                        }
                ));
    ...
    void shouldReturnCriticalInventoryItems_withWarningSeverity() {
        when(productRepository.findCriticalInventory()).thenReturn(List.of(
                buildProjection(1L, "Product A", "SKU-A", BigDecimal.valueOf(3), BigDecimal.valueOf(5))
        ));
    ...
    void shouldReturnCriticalInventoryItems_withCriticalSeverity_whenStockIsZero() {
        when(productRepository.findCriticalInventory()).thenReturn(List.of(
                buildProjection(2L, "Product B", "SKU-B", BigDecimal.ZERO, BigDecimal.valueOf(10))
        ));
    ...
    void shouldReturnMultipleItemsWithMixedSeverity() {
        when(productRepository.findCriticalInventory()).thenReturn(List.of(
                buildProjection(1L, "Product A", "SKU-A", BigDecimal.ZERO, BigDecimal.valueOf(10)),
                buildProjection(2L, "Product B", "SKU-B", BigDecimal.valueOf(3), BigDecimal.valueOf(5)),
                buildProjection(3L, "Product C", "SKU-C", BigDecimal.valueOf(0), BigDecimal.valueOf(1))
        ));
