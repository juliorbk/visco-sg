...
class ProductControllerTest {
  ...
  private ProductDTO createDTO(Long id) {
    return ProductDTO.builder()
      .id(id)
      .internalCode(String.format("%06d", id))
      .sku("SKU-" + id)
      .name("Product " + id)
      .uom("UNIDAD")
      .reorderPoint(BigDecimal.TEN)
      .totalStock(BigDecimal.valueOf(50))
      .totalPendingStock(BigDecimal.ZERO)
      .active(true)
      .build();
  }
  ...
  void getAllProducts_shouldReturn200() throws Exception {
    when(productService.getAllProducts(any(PageRequest.class))).thenReturn(
      new PageImpl<>(List.of(createDTO(1L)))
    );

    mockMvc
      .perform(get("/api/inventory/products"))
  ...
  void createProduct_shouldReturn201() throws Exception {
    when(productService.createProduct(any(ProductDTO.class))).thenReturn(
      createDTO(1L)
    );
    ...
    mockMvc
      .perform(
        post("/api/inventory/products")
          ...
          .content(
            "{\"sku\":\"SKU-NEW\",\"name\":\"New\",\"sapCode\":\"SAP\",\"uom\":\"UNIDAD\",\"reorderPoint\":10}"
          )
      )
  ...
  void createProduct_shouldReturn400_whenDuplicateSku() throws Exception {
    when(productService.createProduct(any(ProductDTO.class))).thenThrow(
      new IllegalArgumentException("El SKU ya existe")
    );
    ...
    mockMvc
      .perform(
        post("/api/inventory/products")
          .contentType(MediaType.APPLICATION_JSON)
          .content(
            "{\"sku\":\"SKU-EX\",\"name\":\"P\",\"sapCode\":\"S\",\"uom\":\"UNIDAD\",\"reorderPoint\":10}"
          )
      )
  ...
  void getProductById_shouldReturn200() throws Exception {
    ...
    mockMvc
      .perform(get("/api/inventory/products/1"))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.sku").value("SKU-1"));
  }
  ...
  void getProductById_shouldReturn404() throws Exception {
    ...
    mockMvc
      .perform(get("/api/inventory/products/99"))
  ...
  void updateProduct_shouldReturn200() throws Exception {
    ...
    dto.setName("Updated");
    when(productService.updateProduct(eq(1L), any(Product.class))).thenReturn(
      dto
    );
  ...
  void updateProduct_shouldReturn400_whenSkuConflict() throws Exception {
    when(productService.updateProduct(eq(1L), any(Product.class))).thenThrow(
      new IllegalArgumentException("El SKU ya existe")
    );
  ...
  void deleteProduct_shouldReturn204() throws Exception {
    ...
    mockMvc
      .perform(delete("/api/inventory/products/1"))
