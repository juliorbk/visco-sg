...
public interface StockLevelRepository extends JpaRepository<StockLevel, Long> {
  ...
  @Query(
    value = "SELECT sl FROM StockLevel sl " +
      ...
      "AND (CAST(:sapCode AS string) IS NULL OR p.sapCode = :sapCode) " +
      "AND (CAST(:sku AS string) IS NULL OR p.sku = :sku)",
    countQuery = "SELECT COUNT(sl) FROM StockLevel sl " +
      ...
      "AND (CAST(:sapCode AS string) IS NULL OR sl.product.sapCode = :sapCode) " +
      "AND (CAST(:sku AS string) IS NULL OR sl.product.sku = :sku)"
  )
