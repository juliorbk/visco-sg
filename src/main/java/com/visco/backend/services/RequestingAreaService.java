...
public class RequestingAreaService {
    ...
    public RequestingArea getAreaById(Long id) {
        return requestingAreaRepository
            .findById(id)
    ...
    public RequestingArea createArea(RequestingArea area) {
        if (requestingAreaRepository.findByName(area.getName()).isPresent()) {
            throw new IllegalArgumentException(
                "Area with name '" + area.getName() + "' already exists"
            );
        }
        CostCenter costCenter = area.getCostCenter();
        if (requestingAreaRepository.findByCostCenter(costCenter).isPresent()) {
            throw new IllegalArgumentException(
                "Area with cost center '" + area.getCostCenter() + "' already exists"
            );
        }
        area.setActive(true);
    ...
    public RequestingArea updateArea(Long id, RequestingArea updated) {
        RequestingArea existing = requestingAreaRepository
            .findById(id)
            .orElseThrow(() -> new EntityNotFoundException("Requesting area not found: " + id));
        ...
        if (
            ...
            throw new IllegalArgumentException(
                "Area with name '" + updated.getName() + "' already exists"
            );
        }
        if (
            !existing.getCostCenter().equals(updated.getCostCenter()) &&
            requestingAreaRepository.findByCostCenter(updated.getCostCenter()).isPresent()
        ) {
            throw new IllegalArgumentException(
                "Area with cost center '" + updated.getCostCenter() + "' already exists"
            );
        }
    ...
    public void deactivateArea(Long id) {
        RequestingArea area = requestingAreaRepository
            .findById(id)
            .orElseThrow(() -> new EntityNotFoundException("Requesting area not found: " + id));
