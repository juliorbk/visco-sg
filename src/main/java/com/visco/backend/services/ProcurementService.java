...
public class ProcurementService {
	...
	public PurchaseOrderResponse createPurchaseOrder(
		...
		// Construir la orden con status PENDING
		...
		// Agregar cada item a la orden y reservar stock pendiente
		for (PurchaseOrderItemRequest itemReq : request.items()) {
			...
			PurchaseOrderItem item = PurchaseOrderItem.builder().purchaseOrder(order)
					.product(product).quantity(itemReq.quantity())
					.unitPrice(itemReq.unitPrice()).build();

			order.getItems().add(item);

			// El stock pendiente aumenta: es mercancía que esperamos recibir
			addPendingStock(product.getId(),
	...
	// Rechaza la orden y revierte el stock pendiente
	@Transactional
	public void rejectOrder(Long orderId) {
		PurchaseOrder order = purchaseOrderRepository.findById(orderId).orElseThrow(() -> new EntityNotFoundException(
				"Purchase order not found: " + orderId));

		if (order.getStatus() != PurchaseOrderStatus.PENDING && order.getStatus() != PurchaseOrderStatus.IN_TRANSIT) {
		...
		// Devolver el stock pendiente (ya no vamos a recibir esta mercancía)
		for (PurchaseOrderItem item : order.getItems()) {
			substractPendingStock(item.getProduct().getId(),
	...
	// PENDING → APPROVED
	// Aprueba la orden para que pueda ser enviada por el proveedor
	@Transactional
	...
	// ─────────────────────────────────────────────────────────────
	// Endpoints del controller que delegan en métodos existentes
	// ─────────────────────────────────────────────────────────────
	...
	// ─────────────────────────────────────────────────────────────
	// Recepción de mercancía (Goods Receipt)
	// ─────────────────────────────────────────────────────────────
	...
	// ordenada
	private boolean determineIfFullyReceived(PurchaseOrder order,
	...
	private GoodReceiptResponse buildReceiptResponse(GoodReceipt receipt, PurchaseOrder order) {
	...
	private void addCurrentStock(Long productId, BigDecimal quantity) {
		List<StockLevel> stockLevels = stockLevelRepository.findByProductId(productId);
	...
	private void substractCurrentStock(Long productId, BigDecimal quantity) {
		List<StockLevel> stockLevels = stockLevelRepository.findByProductId(productId);
	...
	private void substractPendingStock(Long productId, BigDecimal quantity) {
		List<StockLevel> stockLevels = stockLevelRepository.findByProductId(productId);
		...
		List<PurchaseOrderItemResponse> itemResponses = order.getItems()
				...
				.map(item -> new PurchaseOrderItemResponse(
						...
						item.getUnitPrice().multiply(
								BigDecimal.valueOf(item.getQuantity()))))
				.toList();
		return new PurchaseOrderResponse(order.getId(), order.getOrderNumber(),
				order.getDescription(), order.getStatus(),
				order.getSupplier().getName(), order.getCreatedAt(),
				itemResponses);
