...
public class WarehouseService {
    ...
    public GoodReceiptResponse receiveGoods(Long orderId, ReceiveGoodsRequest request) {
        ...
        for (ReceiveGoodsRequest.ReceiveItem itemReq : request.items()) {
            ...
            StockLevel stockLevel = findOrCreateStockLevel(poItem.getProduct(), destWarehouse);
            ...
            substractPendingStock(poItem.getProduct().getId(), destWarehouse.getId(), received);
    ...
    private StockLevel findOrCreateStockLevel(Product product, Warehouse warehouse) {
    ...
    public void transferStock(TransferStockRequest request) {
        ...
        StockLevel sourceStock = stockLevelRepository
            ...
            .orElseThrow(() ->
                new EntityNotFoundException("Stock level not found for source warehouse")
            );
        ...
        sourceStock.setCurrentStock(sourceStock.getCurrentStock().subtract(request.quantity()));
    ...
    public Page<InventoryMovementResponse> getMovements(
        ...
        return movementsPage.map((m) -> {
            ...
            BigDecimal runningBalance = running.updateAndGet((current) -> current.add(qty));
