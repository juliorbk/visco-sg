...
public class WarehouseService {
    ...
    public WarehouseDTO createWarehouse(CreateWarehouseRequest request) {
        User responsible = userRepository
            ...
            .orElseThrow(() ->
                new EntityNotFoundException("User not found: " + request.responsibleUserId())
            );
    ...
    public PurchaseOrderReceiptSummary getReceiptSummaryByOrder(Long orderId) {
        ...
        List<GoodReceipt> receipts = goodReceiptRepository.findByPurchaseOrderIdWithFetch(orderId);
    ...
    public void substractCurrentStock(Long productId, Long warehouseId, BigDecimal quantity) {
        ...
        if (updated == 0) {
            BigDecimal currentStock = stockLevelRepository.getCurrentStock(productId, warehouseId);
    ...
    public void transferStock(TransferStockRequest request) {
        ...
        Warehouse fromWarehouse = warehouseRepository
            ...
            .orElseThrow(() -> new EntityNotFoundException("Source warehouse not found"));

        Warehouse toWarehouse = warehouseRepository
            .findById(request.toWarehouseId())
            .orElseThrow(() -> new EntityNotFoundException("Destination warehouse not found"));
    ...
    public DispatchResponse outputStock(DispatchRequest request, String currentUserEmail) {
        Warehouse warehouse = warehouseRepository
            ...
            .orElseThrow(() ->
                new EntityNotFoundException("Warehouse not found: " + request.warehouseId())
            );

        Employee employee = employeeRepository
            .findById(request.employeeId())
            .orElseThrow(() ->
                new EntityNotFoundException("Employee not found: " + request.employeeId())
            );

        if (!employee.isActive()) {
            throw new IllegalArgumentException("Employee is inactive: " + request.employeeId());
        }
        ...
        for (DispatchRequest.DispatchItem itemReq : request.items()) {
            ...
            substractCurrentStock(product.getId(), warehouse.getId(), itemReq.quantity());
    ...
    public DispatchResponse getDispatchById(Long id) {
        DispatchNote note = dispatchNoteRepository
            .findByIdDetailed(id)
            .orElseThrow(() -> new EntityNotFoundException("Dispatch not found: " + id));
        return DispatchResponse.fromEntity(note);
    }
    ...
    public Page<InventoryMovementResponse> getMovements(
        ...
        return movementsPage.map((m) -> {
            ...
            BigDecimal runningBalance = running.updateAndGet((current) -> current.add(signedQty));
